...
SceneFileParser.prototype.setViewport = function () {
    this.mViewport = gEngine.ResourceMap.retrieveAsset("Viewport");
    this.Cx2 = this.mViewport[0];
    this.Cy2 = this.mViewport[1];
    
};


SceneFileParser.prototype.parseCamera2 = function () {
    var camElm = this._getElm("Camera");
    var cx = this.Cx2;
    var cy = this.Cy2;
    cx = Number(camElm[0].getAttribute("CenterX"));
    cy = Number(camElm[0].getAttribute("CenterY"));
    var w = Number(camElm[0].getAttribute("Width")/2);
